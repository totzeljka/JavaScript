...
describe("createAccount", () => {
    let newEmailAddress = "newuser@pluralsight.com";

    beforeEach(() => {
        jest.spyOn(users, 'userExists').mockReturnValue(false);
    ...
    test("Returns user data when account created successfully- resolves", () => {
        expect.hasAssertions();
        return expect(account.createAccount(newEmailAddress)).resolves.toStrictEqual({
            data: { userId: 2, username: newEmailAddress },
        });
    });

    test("Returns error when user already exists", () => {
    ...
    test("Returns error when user already exists - async/await", async () => {
        jest.spyOn(users, 'userExists').mockReturnValue(true);
        expect.hasAssertions();
